...
export function ProfilePageClient({ profile, sessionUser }: ProfilePageClientProps) {
  ...
  const [selectedUpgradeTier, setSelectedUpgradeTier] = useState<'premium' | 'pro'>('premium');
  ...
  const currentPriceLabel =
    tier === 'basic' ? 'Rp 99rb/bulan' : tier === 'premium' ? 'Rp 99rb/bulan' : tier === 'pro' ? 'Rp 179rb/bulan' : 'Rp 0';

  const TIER_ORDER: Record<SubscriptionTier, number> = { free: 0, basic: 1, premium: 1, pro: 2 };

  useEffect(() => {
    if (upgradeTierParam && ['premium', 'pro'].includes(upgradeTierParam)) {
      setSelectedUpgradeTier(upgradeTierParam as 'premium' | 'pro');
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              {isPaidTier && (
                <span className="flex items-center gap-1 bg-amber-50 text-amber-600 text-xs font-semibold px-2 py-0.5 rounded-full border border-amber-200">
                  ...
                  {tier === 'basic' || tier === 'premium' ? 'Premium' : 'Pro'}
                </span>
              )}
            </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t">
          {[
            ...
            },
            {
              icon: User,
              label: 'Tipe Akun',
              value: tier === 'free' ? 'Gratis' : tier === 'basic' || tier === 'premium' ? 'Premium' : 'Pro',
            },
            {
              icon: TrendingUp,
              label: 'Bergabung',
      ...
      <div className="card p-6 mb-4" id="premium">
        ...
        {isPaidTier || subscriptionStatus === 'expired' ? (
          <div className="flex items-center gap-4 bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-4">
            ...
            <div className="flex-1">
              <p className="font-bold text-amber-700">
                Propti {tier === 'basic' || tier === 'premium' ? 'Premium' : 'Pro'}
              </p>
          ...
          <div className="bg-gray-50 rounded-2xl p-4 mb-4">
            ...
            <ul className="space-y-2 text-sm text-gray-600">
             {['5 listing aktif', 'Maksimal 5 foto per iklan', 'Iklan tayang 60 hari'].map((f) => (
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          {(
            ...
            const isCurrent = tier === plan.key || (plan.key === 'premium' && tier === 'basic');
