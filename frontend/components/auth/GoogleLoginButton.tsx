...
export function GoogleLoginButton({ callbackUrl = '/' }: { callbackUrl?: string }) {
  ...
  const handleLogin = async () => {
    setLoading(true);
    await signIn('google', { callbackUrl: buildPostLoginCallbackUrl(callbackUrl) });
  };
  ...
  return (
    <div className="space-y-3">
      <button
        onClick={handleLogin}
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-200 text-gray-700 font-semibold py-3.5 px-6 rounded-2xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 shadow-sm hover:shadow-md disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {loading ? (
