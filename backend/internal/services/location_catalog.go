...
// maxSearchResults caps results when the caller supplies a non-empty query
// (autocomplete / type-ahead). When the query is empty all items are returned
// so that dropdowns can be fully populated without multiple round trips.
...
func (c *LocationCatalog) SearchProvinces(query string) []Province {
	...
	for _, p := range c.provinces {
		if strings.Contains(strings.ToLower(p.Name), q) {
			...
			if query != "" && len(results) == maxSearchResults {
				break
			}
		}
	}
