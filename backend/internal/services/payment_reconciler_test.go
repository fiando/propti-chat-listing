...
func TestPaymentReconcilerUpgradesUserWhenPendingPremiumPaymentIsPaid(t *testing.T) {
	...
	txStore := &fakeReconcileTransactionStore{items: []models.Transaction{{
		...
		PaymentID:     "tok-123",
		Metadata:      map[string]string{"tier": string(models.SubscriptionBasic)},
	}}}
	...
	if got := userStore.user.Subscription.Tier; got != models.SubscriptionBasic {
		t.Fatalf("expected basic tier, got %q", got)
	}
...
func TestPaymentReconcilerDefaultsToPremiumTierWithoutMetadata(t *testing.T) {
	t.Parallel()
	...
	txStore := &fakeReconcileTransactionStore{items: []models.Transaction{{
		...
		Status:        models.TransactionStatusPending,
		Provider:      payments.ProviderDOKU,
		PaymentID:     "tok-legacy",
	}}}
	reconciler := NewPaymentReconciler(userStore, txStore, &fakeReconcileProvider{status: payments.PaymentStatusSucceeded})

	if err := reconciler.ReconcileUser(context.Background(), "user-2"); err != nil {
		t.Fatalf("ReconcileUser returned error: %v", err)
	}

	if got := userStore.user.Subscription.Tier; got != models.SubscriptionPremium {
		t.Fatalf("expected default premium tier for legacy tx, got %q", got)
	}
}
