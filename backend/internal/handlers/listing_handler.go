...
type listingService interface {
	...
	RenewListing(ctx context.Context, userID, listingID string) (*models.Listing, error)
...
func (h *ListingHandler) Handle(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	case method == http.MethodPost && isListingRenewPath(path):
		return h.renewListing(ctx, req)
...
func (h *ListingHandler) renewListing(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	userID, err := extractUserID(req)
	if err != nil {
		return jsonResponse(http.StatusUnauthorized, utils.MarshalErrorResponse(utils.ErrUnauthorized)), nil
	}
	...
	listing, err := h.listingService.RenewListing(ctx, userID, listingID)
	if err != nil {
		return appErrorResponse(err), nil
	}
	return h.ownerListingResponse(ctx, http.StatusOK, listing)
...
func isListingRenewPath(path string) bool {
	parts := strings.Split(strings.Trim(path, "/"), "/")
	return len(parts) == 3 && parts[0] == "listings" && parts[1] != "" && parts[2] == "renew"
}
